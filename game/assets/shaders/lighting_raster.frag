...
void main() {
	...
	const vec3 normal = normalFresnel.xyz;
	const float fresnel = normalFresnel.a;
	
	vec3 lighting = BasicLighting(lightPushConstant.lightSource, lightPushConstant.viewSpacePosition, GetFragViewSpacePosition(), normal, fresnel, metallic, roughness);
	
	if ((camera.debug & RENDER_DEBUG_FORCE_UNIFORM_SUNLIGHT) != 0) {
		lighting = vec3(min(lighting.r, min(lighting.g, lighting.b)));
	}
	
	out_light = vec4(lighting, 1);
}
