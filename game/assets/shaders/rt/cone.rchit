...
hitAttributeEXT ConeAttr coneAttr;
	...
	void main() {
		...
		// Optionally have a second radius from an additional custom float
		// If it is positive use it as the B end radius (at aabb_max), if negative use its absolute as the A end radius (at aabb_min)
		// Not using the additional custom float will result in a perfect cone
		float rb = 0; //TODO put Second Radius here if applicable
		if (rb < 0) {
			float tmp = -rb;
			rb = ra;
			ra = tmp;
		...
		// Ray-Cone Intersection with caps (ro, rd, pa, pb, ra, rb)
		const vec3 ba = pb - pa;
		const vec3 oa = ro - pa;
		const vec3 ob = ro - pb;
		...
		if (m1 < 0.0) {
			if (dot2(oa*m3-rd*m1) < (ra*ra*m3*m3)) {
				...
				if (gl_RayTminEXT <= t) {
					...
					coneAttr.normal = normalize(-ba*inversesqrt(m0));
		...
		if (m1 > 0.0) {
			if (dot2(-oa*m3+rd*m1) < (ra*ra*m3*m3)) {
				...
				if (gl_RayTminEXT <= t) {
					...
					coneAttr.len = length(ba);
					coneAttr.normal = normalize(+ba*inversesqrt(m0));
					reportIntersectionEXT(t, 4);
					return;
				}
			}
		}
		
		// Inside surface of B cap
		if (m2 < 0.0) {
			if (dot2(-ob*m3+rd*m2) < (rb*rb*m3*m3)) {
				const float t = m2/-m3;
				if (gl_RayTminEXT <= t) {
	...
	void main() {
		RCHIT_VISIBILITY_BEGIN
			...
			ray.normal = normalize(mat3(MODELVIEW) * coneAttr.normal);
