...
TerrainClutterAabbData rock = TerrainClutterAabbData(GetRenderableInstance().instanceBufferPtr)[gl_PrimitiveID];
vec3 aabb_min = vec3(rock.aabb[0], rock.aabb[1], rock.aabb[2]);
vec3 aabb_max = vec3(rock.aabb[3], rock.aabb[4], rock.aabb[5]);
...
int detailOctavesHighRes = int(round(5*smoothstep(camera.terrainClutterDetail*5, 0, approxDistanceFromCamera)));
...
float fadeDistance = camera.terrainClutterDetail*10;
...
float minSizeInScreen = smoothstep(5.0, 0.01, camera.terrainClutterDetail) + 1;

float GetFixedBoundingSizeInScreenPixels(float distance, float size) {
	float solidAngle;
	if (distance == 0 || camera.fov == 0) solidAngle = PI;
	else solidAngle = size / distance / radians(float(camera.fov));
	return min(solidAngle, PI) * 2.0 * ((camera.width + camera.height) * 0.5);
}

float GetDetailSize() {
	uint seed = rock.seed;
	return mix(minDetailSize, maxDetailSize, RandomFloat(seed));
}

float Sdf(vec3 p, float detailSize, int detailOctaves) {
	uint seed = rock.seed;
	float rnd1 = RandomFloat(seed);
	float rnd2 = RandomFloat(seed);
	...
	for (int x = -1; x <= 1; ++x) {
		for (int z = -1; z <= 1; ++z) if (x != 0 || z != 0) {
			...
			float rotation = RandomFloat(seed) * 2 * PI;
			vec3 plane = vec3(
				pt.x * RandomFloat(seed),
				pt.y + RandomFloat(seed) * rockSize.y,
				pt.z * RandomFloat(seed)
			);
...
void main() {
	RCHIT_VISIBILITY_BEGIN
		vec3 pos = ray.vertexPosition - rockPos;
		...
		ray.color = vec4(0.6,0.5,0.4, 1);
		ray.color.rgb *= mix(0.3, 1.0, abs(FastSimplexFractal(ray.vertexPosition*759.658, detailOctavesTextures)));
		ray.color.rgb *= pow(clamp(-normal.y, 0, 1), 0.333);
		ray.pbr = vec2(0, clamp(FastSimplexFractal(ray.vertexPosition*223.18, detailOctavesTextures), 0.1, 1));
		ray.emission = vec3(0);
		ray.normal = normalize(mat3(MODELVIEW) * normal);
		// ray.bump = ray.normal * detailSize;
		ray.bump = vec3(0);
		ray.iOR = 1.45;
