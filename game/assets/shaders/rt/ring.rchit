...
hitAttributeEXT ConeRingAttr coneRingAttr;
	...
	void main() {
		...
		// Not using the additional custom float will result in a perfect cylinder-ring
		float rb = ra; //TODO put Second Radius here if applicable, or use ra
		if (rb < 0) {
			float tmp = -rb;
			rb = ra;
			ra = tmp;
		...
		if (y1 > 0.0 && y1 < m0) {
			if (gl_RayTminEXT <= t1) {
				coneRingAttr.ra = ra;
				coneRingAttr.rb = rb;
				coneRingAttr.len = length(ba);
				coneRingAttr.normal = normalize(m0*(m0*(oa+rd*t1)+rr*ba*ra) - ba*hy*y1);
		...
		if (y2 > 0.0 && y2 < m0) {
			if (gl_RayTminEXT <= t2) {
				...
				coneRingAttr.rb = rb;
				coneRingAttr.len = length(ba);
				coneRingAttr.normal = -normalize(m0*(m0*(oa+rd*t2)+rr*ba*ra) - ba*hy*y2);
	...
	void main() {
		RCHIT_VISIBILITY_BEGIN
			...
			ray.normal = normalize(mat3(MODELVIEW) * coneRingAttr.normal);
