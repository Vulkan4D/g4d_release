	...
	void main() {
		RCHIT_VISIBILITY_BEGIN
			...
			vec2 uv = (
				...
				ray.pbr = vec2(GetMaterialMetallic(), GetMaterialRoughness());
			#endif
			
			// Surfaces that are both translucent and metallic will not be translucent if seen from the front (Helmet glass)
			if (ray.pbr.r > 0.0 && ray.color.a < 1.0) {
				float nDotR = dot(ray.normal, gl_WorldRayDirectionEXT);
				if (nDotR < 0.0) ray.color.a = 1;
				else ray.pbr.rg = vec2(0,1);
			}
