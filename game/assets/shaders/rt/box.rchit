...
const float EPSILON = 0.00001;
	...
	void main() {
		RCHIT_VISIBILITY_BEGIN
			...
			const float THRESHOLD = EPSILON * ray.totalDistanceFromEye;
			const vec3 absMin = abs(ray.vertexPosition.xyz - (gl_HitKindEXT == 0? aabb_min.xyz : aabb_max.xyz));
			const vec3 absMax = abs(ray.vertexPosition.xyz - (gl_HitKindEXT == 0? aabb_max.xyz : aabb_min.xyz));
			vec3 normal;
				 if (absMin.x < THRESHOLD) normal.xyz = vec3(-1, 0, 0);
			else if (absMin.y < THRESHOLD) normal.xyz = vec3( 0,-1, 0);
			else if (absMin.z < THRESHOLD) normal.xyz = vec3( 0, 0,-1);
			else if (absMax.x < THRESHOLD) normal.xyz = vec3( 1, 0, 0);
			else if (absMax.y < THRESHOLD) normal.xyz = vec3( 0, 1, 0);
			else if (absMax.z < THRESHOLD) normal.xyz = vec3( 0, 0, 1);
