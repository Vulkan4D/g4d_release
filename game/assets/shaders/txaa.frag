...
void main() {
	float maxAccumulation = TXAA_SAMPLES;
	vec2 uv = GetFragUV();
	if ((camera.options & RENDER_OPTION_TXAA) != 0 && (camera.options & RENDER_OPTION_DLSS) == 0 && camera.frameIndex > 2) {
		vec4 lit = texture(sampler_lit, uv);
		vec2 uvHistory = uv;
		if (ReprojectHistoryUV(uvHistory)) {
			vec4 history = texture(sampler_history, uvHistory);
			history.rgb = VarianceClamp9(history.rgb, sampler_lit, uv);
			float accumulation = min(history.a, maxAccumulation) + 1;
			float blend = clamp(1.0 / accumulation, 0, 1);
			out_resolved = vec4(mix(history.rgb, lit.rgb, blend), accumulation);
		} else {
			out_resolved = vec4(lit.rgb, 0);
		}
	} else {
		out_resolved = vec4(texture(sampler_lit, uv).rgb, 0);
	}
}
