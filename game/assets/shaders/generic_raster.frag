...
layout(location = 5) in vec4 in_pos_history;
...
void main() {
	...
	if (geometry.albedo_opacity.a < 0.5) {
		discard;
	}
	float depth = gl_FragCoord.z/gl_FragCoord.w;
	vec3 normal = normalize(MODELVIEWNORMAL * GetMaterialNormal(in_normal, in_tangent, in_uv));
	out_color = vec4(in_color.rgb * GetMaterialAlbedo(in_uv).rgb * geometry.albedo_opacity.rgb, geometry.albedo_opacity.a);
	out_normalAndFresnel = vec4(normal, Fresnel(GetViewSpacePosition(), normal, GetMaterialIndexOfRefraction()));
