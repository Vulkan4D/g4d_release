...
vec4 StarsBlurred(in samplerCube tex, in vec3 p) {
	const float size = textureSize(tex, 0).x * 0.5;
	vec4 color = texture(tex, p);
	
	color = max(color, texture(tex, p + vec3(1,0,0)/size) * 0.7);
	color = max(color, texture(tex, p + vec3(0,1,0)/size) * 0.7);
	color = max(color, texture(tex, p + vec3(0,0,1)/size) * 0.7);
	color = max(color, texture(tex, p + vec3(-1,0,0)/size) * 0.7);
	color = max(color, texture(tex, p + vec3(0,-1,0)/size) * 0.7);
	color = max(color, texture(tex, p + vec3(0,0,-1)/size) * 0.7);
	
	color = max(color, texture(tex, p + vec3(2,0,0)/size) * 0.2);
	color = max(color, texture(tex, p + vec3(0,2,0)/size) * 0.2);
	color = max(color, texture(tex, p + vec3(0,0,2)/size) * 0.2);
	color = max(color, texture(tex, p + vec3(-2,0,0)/size) * 0.2);
	color = max(color, texture(tex, p + vec3(0,-2,0)/size) * 0.2);
	color = max(color, texture(tex, p + vec3(0,0,-2)/size) * 0.2);
...
void main() {
	...
	vec4 fog = imageLoad(img_fog, coord);
	vec3 emission = imageLoad(img_emission, coord).rgb;
	vec4 color = subpassLoad(in_img_color);
	
	// Apply emission
	vec3 lit = emission;
	
	if (depth == 0) {
		// Apply Background stars
		vec3 rayDirection = normalize(vec4(inverse(mat4(camera.projectionMatrix)) * vec4(uv*2-1, 1, 1)).xyz);
		vec3 lookDir = normalize(transpose(mat3(camera.viewMatrix * camera.inverseGalaxyWorld)) * rayDirection);
		// lit += StarsBlurred(sampler_background, lookDir).rgb*0.01;
		
		// Write motion vectors for the Sky
		imageStore(img_motion, coord, vec4(GetMotionVector(uv, depth), 0));
		
	} else {
		// Apply Geometry and Lighting
		vec3 light = imageLoad(img_light, coord).rgb;
		vec3 color = subpassLoad(in_img_color).rgb;
		lit += light * color;
	}
	
	// Apply Fog
	lit = mix(lit, fog.rgb, fog.a);
	
	if ((camera.debug & RENDER_DEBUG_LIGHTS) != 0) {
		lit = imageLoad(img_light, coord).rgb * length(subpassLoad(in_img_color).rgb);
	}

	// Store final lit image
	imageStore(img_lit, coord, vec4(lit, /*FutureUsage*/0));
}
