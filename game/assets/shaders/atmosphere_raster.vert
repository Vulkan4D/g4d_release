...
void GetProjectedBounds(vec3 center, float radius, inout vec3 boxMin, inout vec3 boxMax) {
	vec4 east = camera.projectionMatrixWithTXAA * vec4(center + vec3(+radius,0,0) , 1);
	vec4 west = camera.projectionMatrixWithTXAA * vec4(center + vec3(-radius,0,0) , 1);
	vec4 north = camera.projectionMatrixWithTXAA * vec4(center + vec3(0,+radius,0) , 1);
	vec4 south = camera.projectionMatrixWithTXAA * vec4(center + vec3(0,-radius,0) , 1);
...
void main() {
	const float r = pushConstant.radius*(1+radians(camera.fov));
	if (length(pushConstant.modelViewMatrix[3].xyz) < r) {
		...
		vec3 boxMin, boxMax;
		GetProjectedBounds(pushConstant.modelViewMatrix[3].xyz, r, boxMin, boxMax);
		if (boxMin.z < 0 && boxMax.z < 0) return;
		const vec4 position[4] = {
			vec4(boxMin.x, boxMin.y, 0,1),
			vec4(boxMin.x, boxMax.y, 0,1),
			vec4(boxMax.x, boxMin.y, 0,1),
			vec4(boxMax.x, boxMax.y, 0,1)
		};
		gl_Position = position[gl_VertexIndex];
	}
}
