...
void main() {
	...
	if ((camera.options & RENDER_OPTION_RAY_TRACED_VISIBILITY) != 0) {
		if (imageLoad(img_rtDepth, scaledCoord).r != 0) discard;
	} else {
		if (texture(sampler_rasterDepth, scaledCoord / vec2(camera.width, camera.height)).r != 0) discard;
	}
	float fog = clamp((length(imageLoad(img_emission, scaledCoord).rgb) + length(imageLoad(img_fog, scaledCoord)))*0.5, 0, 1);
	float center = max(0, 1.0 - pow(length(gl_PointCoord * 2 - 1), 2));
	out_color = vec4(ApplyToneMapping(in_color.rgb * in_color.a * 0.00004 * pow(center, 2)), in_color.a) * (1.0-fog);
}
