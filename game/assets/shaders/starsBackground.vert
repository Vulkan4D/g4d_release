...
// #define USE_VERTEX_INPUTS
// #include "v4d/game/galaxy/glsl/background.glsl"
// #shader vert
// #shader geom
// #shader frag
...
void main() {
	...
	vec4 pointColor = vec4(in_color.rgb, clamp(pow(in_color.a * (dd + dd2) * brightnessFactor, 0.5), 0.0, 100.0));
	
	uint i = gl_VertexIndex;
	gl_Position = camera.projectionMatrix * camera.viewMatrix * camera.inverseGalaxyWorld * vec4((pos + mix(vec3(RandomFloat(i), RandomFloat(i), RandomFloat(i))*2-1, vec3(0), dd)) * 1e16, 1);
	gl_PointSize = max(3.0, in_position.w * sizeFactor);
	
	// Compensate for darkening on some monitors when moving
	if (camera.starBrightnessCompensationForMonitorDuringMovement > 1.001) {
		vec2 ndc = gl_Position.xy / gl_Position.w;
		vec2 screenSpacePos = ndc.xy * 0.5 + 0.5;
		vec2 motion = imageLoad(img_motion, ivec2(screenSpacePos * camera.renderScale)).xy;
		if (abs(motion.x) + abs(motion.y) > 0) gl_PointSize *= camera.starBrightnessCompensationForMonitorDuringMovement;
	}
