...
const bool humanEyeExposure = true;

void main() {
	vec2 uv = GetFragUV() * camera.renderScale;
	vec4 post = texture(sampler_post, uv);
	float depth = GetDepth(uv);
	ivec2 scaledCoord = ivec2(gl_FragCoord.st * camera.renderScale);
	
	switch (camera.debugViewMode) {
		default: case 0:
			out_swapchain = vec4(post.rgb, 1);
			break;
		case RENDER_DEBUG_VIEWMODE_COLOR:
			out_swapchain = vec4(imageLoad(img_color, scaledCoord).rgb, 1);
			break;
		case RENDER_DEBUG_VIEWMODE_NORMAL:
			out_swapchain = vec4(mix(vec3(0.5), imageLoad(img_normal, scaledCoord).rgb, 4.0), 1);
			break;
		case RENDER_DEBUG_VIEWMODE_DEPTH:
			out_swapchain = vec4(Heatmap(pow(depth, 0.1)), 1);
			break;
		case RENDER_DEBUG_VIEWMODE_DISTANCE:
			float dist;
			if ((camera.options & RENDER_OPTION_DLSS) != 0) {
				dist = length(GetViewSpacePositionFromDepthAndUV(depth, uv));
			} else {
				dist = texture(sampler_resolved, uv).a;
			}
			out_swapchain = vec4(Heatmap(pow(smoothstep(0.1, 1000000, dist), 0.1)), 1); // True distance from camera (black = 10 cm, white = 1000 km)
			break;
		case RENDER_DEBUG_VIEWMODE_METALLIC:
			out_swapchain = vec4(vec3(imageLoad(img_pbr, scaledCoord).r), 1);
			break;
		case RENDER_DEBUG_VIEWMODE_ROUGHNESS:
			out_swapchain = vec4(vec3(imageLoad(img_pbr, scaledCoord).g), 1);
			break;
		case RENDER_DEBUG_VIEWMODE_FRESNEL:
			out_swapchain = vec4(vec3(imageLoad(img_normal, scaledCoord).a), 1);
			break;
		case RENDER_DEBUG_VIEWMODE_LIGHTING:
		case RENDER_DEBUG_VIEWMODE_RT_TIME:
			out_swapchain = vec4(imageLoad(img_light, scaledCoord).rgb, 1);
			break;
		case RENDER_DEBUG_VIEWMODE_MOTION:
			out_swapchain = vec4(abs(texture(sampler_motion, uv).xyz) * vec3(20,20, 20000), 1); // Motion Vector
			break;
		case RENDER_DEBUG_VIEWMODE_EMISSION:
			out_swapchain = vec4(imageLoad(img_emission, scaledCoord).rgb, 1);
			break;
		case RENDER_DEBUG_VIEWMODE_FOG:
			out_swapchain = vec4(imageLoad(img_fog, scaledCoord).rgb * imageLoad(img_fog, scaledCoord).a, 1);
			break;
		case RENDER_DEBUG_VIEWMODE_THUMBNAIL:
			ivec2 screenSize = textureSize(sampler_post, 0);
			ivec2 thumbnailSize = imageSize(thumbnails[0]);
			ivec2 coord = ivec2(gl_FragCoord.st);
			if (coord.x > screenSize.x - thumbnailSize.x && coord.y > screenSize.y - thumbnailSize.y) {
				out_swapchain = vec4(imageLoad(thumbnails[0], coord - screenSize + thumbnailSize).rgb, 1);
			} else {
				out_swapchain = vec4(post.rgb, 1);
			}
			break;
