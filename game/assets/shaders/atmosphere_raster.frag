...
bool RaymarchAtmosphere(in vec3 origin, in vec3 viewDir, in float hitDistance, in vec3 position, in float t1, in float t2) {
	CelestialInstanceData celestial = GetCelestialInstance();
	float innerRadius = celestial.innerRadius;
	float outerRadius = celestial.outerRadius;
	vec4 rayleigh = celestial.atmosphereRayleigh;
	vec4 mie = celestial.atmosphereMie;
	float sunGlow = celestial.atmosphereSunGlow;
	
	float startAltitude = distance(origin, position);
	float thickness = outerRadius - innerRadius;
	
	float inner_t1, inner_t2;
	bool hitInnerRadius = RaySphereIntersection(position, viewDir, innerRadius, inner_t1, inner_t2);
	...
	bool hasHitSomethingWithinAtmosphere = hitDistance < t2;
	const vec3 startPointViewSpace = origin + viewDir * t1;
	const float rayStartAltitude = length(startPointViewSpace - position);
	vec3 endPointViewSpace = origin + viewDir * min(hitDistance, t2);
	float rayDepth = distance(startPointViewSpace, endPointViewSpace);
	...
	float maxDepth = 0;
	for (int sunIndex = 0; sunIndex < NB_SUNS; ++sunIndex) {
		SunlightSource sun = SunlightSource(uint64_t(celestial))[sunIndex];
		vec3 lightIntensity = sun.color * sun.luminosity * 4.0 * 3.141592654;
		if ((camera.debug & RENDER_DEBUG_FORCE_UNIFORM_SUNLIGHT) != 0) {
			lightIntensity = vec3(min(lightIntensity.r, min(lightIntensity.g, lightIntensity.b)));
		}
		if (length(lightIntensity) > sunLuminosityThreshold) {
			vec3 lightPosViewSpace = vec4(camera.viewMatrix * dvec4(sun.position, 1)).xyz;
			...
			float mu = dot(viewDir, lightDirViewSpace);
			float mumu = mu * mu;
			...
			// Ray-March
			vec3 rayPosViewSpace = startPointViewSpace;
			for (int i = 0; i < RAYMARCH_STEPS; ++i) {
				rayPosViewSpace += viewDir * stepSize;
				vec3 posOnSphere = rayPosViewSpace - position;
				...
				float a = dot(lightDirViewSpace, lightDirViewSpace);
				float b = 2.0 * dot(lightDirViewSpace, posOnSphere);
				...
				for (int l = 0; l < RAYMARCH_LIGHT_STEPS; ++l) {
					vec3 posLightRay = posOnSphere + lightDirViewSpace * (lightRayDist + lightRayStepSize/2.0);
	...
	emission.rgb += GetEmissionColor(celestial.surfaceTemperature);
	emission.rgb *= 0.5;
	fog = vec4(emission.rgb + rayleigh.rgb*0.01, pow(clamp(maxDepth/thickness, 0, 1), 2.718/*e*/));
	fog.a = mix(0, fog.a, pow(clamp(hitDistance/mie.a, 0, 1), 0.1));
	
	#ifdef SHADER_RAYTRACING
		float fogDepth = (t1 + t2) * 0.5;
		if (fogDepth <= fogRay.fogDepth) {
			fogRay.emission = max(vec4(0), emission) + fogRay.emission * (1-fog.a);
			fogRay.fog = vec4(fogRay.fog.rgb * fogRay.fog.a * (1-fog.a) + fog.rgb * fog.a, fog.a); // Standard Alpha Blending (When drawing from Back to Front)
		} else {
			fogRay.emission = fogRay.emission + max(vec4(0), emission) * (1-fogRay.fog.a);
			fogRay.fog = vec4(fog.rgb * fog.a * (1-fogRay.fog.a) + fogRay.fog.rgb, fogRay.fog.a); // Reversed Alpha Blending (When drawing from Front to Back)
		}
		fogRay.fogDepth = fogDepth;
		if (max(fogRay.fog.a, fog.a) >= 0.99) fogRay.opaqueDepth = fogDepth;
	#else
		out_fog = fog;
		out_emission = max(vec4(0), emission);
	#endif
...
void main() {
	if ((camera.options & RENDER_OPTION_RAY_TRACED_VISIBILITY) != 0) discard;
	
	vec2 uv = GetFragUV();
	
	// Ray-Sphere intersection
	const vec3 viewDir = normalize(vec4(inverse(mat4(camera.projectionMatrixWithTXAA)) * vec4(uv*2-1, 1, 1)).xyz);
	const vec3 position = pushConstant.modelViewMatrix[3].xyz;
	
	float t1,t2;
	if (!RaySphereIntersection(position, viewDir, pushConstant.radius, t1, t2)) discard;
	
	float hitDistance = length(GetFragViewSpacePosition(uv));
	if (hitDistance <= 0) hitDistance = float(camera.zFar);
	if (hitDistance < t1) discard;
	
	if (!RaymarchAtmosphere(vec3(0), viewDir, hitDistance, position, max(0, t1), t2)) {
		discard;
	}
}
