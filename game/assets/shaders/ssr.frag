...
void main() {
	if (camera.frameIndex <= 2) discard;
	
	const vec2 metallicRoughness = subpassLoad(in_img_pbr).st;
	const float metallic = metallicRoughness.s;
	const float roughness = metallicRoughness.t;
	const vec4 normalFresnel = subpassLoad(in_img_normal);
	const vec3 normal = normalFresnel.rgb;
	const float fresnel = normalFresnel.a;
	
	float reflection = max(metallic, fresnel);
	if (reflection < 0.1) discard;
	float reflectionBlur = (1-fresnel)*roughness*roughness;
	float mirror = clamp(reflection-reflectionBlur, 0, 1);
	
	const vec3 startViewSpacePos = GetFragViewSpacePosition();
	...
	for (int i = 0; i < maxSteps; ++i) {
		...
		if (-viewSpacePos.z > GetTrueDistanceFromDepthBuffer(GetDepth(uv.xy))) {
			break;
		}
	}
	
	if (uv.x <= 0 || uv.x > 1 || uv.y <= 0 || uv.y > 1) discard;
	if (reflection < 0.01) discard;
	
	uint seed = InitRandomSeed(camera.seed, InitRandomSeed(uint(gl_FragCoord.x), uint(gl_FragCoord.y)));
	
	vec3 reflected1 = texture(sampler_history, uv + RandomInUnitSquare(seed)*fragSize*100.0*reflectionBlur).rgb;
	vec3 reflected2 = texture(sampler_history, uv + RandomInUnitSquare(seed)*fragSize*100.0*reflectionBlur).rgb;
	vec3 reflected3 = texture(sampler_history, uv + RandomInUnitSquare(seed)*fragSize*100.0*reflectionBlur).rgb;
	vec3 reflected4 = texture(sampler_history, uv + RandomInUnitSquare(seed)*fragSize*100.0*reflectionBlur).rgb;
	vec3 reflected = clamp((texture(sampler_history, uv).rgb + reflected1 + reflected2 + reflected3 + reflected4) / 5.0, 0, camera.luminance.a);
	
	vec4 lit = imageLoad(img_lit, ivec2(gl_FragCoord.st));
	imageStore(img_lit, ivec2(gl_FragCoord.st), vec4(lit.rgb + reflected*reflection*0.5, lit.a));
}
