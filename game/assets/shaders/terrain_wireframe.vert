...
#define WIREFRAME_COLOR vec4(1,0,0,1) // Red
#define WIREFRAME_VERT_RAW_POS
#define USE_TESSELLATION
#define SHADER_FIRST_VISIBILITY_PASS
#define SHADER_WIREFRAME
...
// Vertex shader
	...
	void main() {
		...
		#ifdef WIREFRAME_VERT_RAW_POS
			gl_Position = vec4(GetVertexPosition(), 1);
		#else
			gl_Position = MVP * vec4(GetVertexPosition(), 1);
		#endif
	}
#endif
...
const float tessellationMinDistance = 1.0;
const float tessellationMaxDistance = 25.0;
float tessellationDisplacementFactor = camera.tessellationDisplacement;

const bool rotateUVs = true;
const float textureRotationAmount = 0.1;
const float textureOffsetAmount = 0.7;

const float tex0_displacement_factor = 1.0;
const float tex1_displacement_factor = 1.0;
const float tex2_displacement_factor = 1.0;
const float tex3_displacement_factor = 1.0;
...
const uint tex_albedo_offset = 0;
const uint tex_normal_offset = 1;
const uint tex_metallic_offset = 2;
const uint tex_roughness_offset = 3;
const uint tex_height_offset = 4;

const vec3 ObjectSpaceUP = vec3(0,-1,0);
const vec3 ObjectSpaceGravityDir = vec3(0,+1,0);

const float textureDisturbanceFadeDistance = 1;
const float textureDisturbanceMaxDistance = 20;

float BumpyNoise(vec3 pos) {
	return clamp(
		+(1-abs(FastSimplex(pos*200)))*0.5
		+(FastSimplex(pos*510)*0.5+.5)
	, 0, 1);
}

#define DECLARE_TERRAIN_VERTEX_OUTPUTS\
	layout(location = 0) out vec3 out_normal;\
	layout(location = 1) out vec2 out_uv;\
	layout(location = 2) out vec4 out_tangent;\
	layout(location = 3) out vec4 out_pos;\
	layout(location = 4) out vec4 out_pos_history;\
	layout(location = 5) out flat uvec4 out_tex_albedo;\
	layout(location = 6) out flat uvec4 out_tex_normal;\
	layout(location = 7) out flat uvec4 out_tex_metallic;\
	layout(location = 8) out flat uvec4 out_tex_roughness;\
	layout(location = 9) out flat uvec4 out_tex_height;\
	layout(location = 10) out vec4 out_blending;\
	layout(location = 11) out vec3 out_posOnChunk;\
	layout(location = 12) out vec3 out_upDir;\

#define DECLARE_TERRAIN_FRAGMENT_INPUTS\
	layout(location = 0) in vec3 in_normal;\
	layout(location = 1) in vec2 in_uv;\
	layout(location = 2) in vec4 in_tangent;\
	layout(location = 3) in vec4 in_pos;\
	layout(location = 4) in vec4 in_pos_history;\
	layout(location = 5) in flat uvec4 in_tex_albedo;\
	layout(location = 6) in flat uvec4 in_tex_normal;\
	layout(location = 7) in flat uvec4 in_tex_metallic;\
	layout(location = 8) in flat uvec4 in_tex_roughness;\
	layout(location = 9) in flat uvec4 in_tex_height;\
	layout(location = 10) in vec4 in_blending;\
	layout(location = 11) in vec3 in_posOnChunk;\
	layout(location = 12) in vec3 in_upDir;\

vec4 GetTextureBlending(in vec4 in_blending, in vec2 in_uv, in uvec4 in_tex_height, in vec3 in_upDir, in vec3 in_posOnChunk, in uint16_t in_flags, in uint8_t in_biomes, in uint8_t in_features, in vec3 in_normal, out vec3 triplanarCoords1, out vec4 triplanarCoords2, out vec3 triplanarBlending) {
	...
	if (rotateUVs && (in_flags & TERRAIN_VERTEX_FLAG_ROTATE_TEXTURES) != 0) {
		TerrainGeometryData chunk = GetTerrainGeometry();
...
void ApplyTerrainTextures(
	...
	in vec2 in_uv,
	in vec4 in_tangent,
	in uvec4 in_tex_albedo,
	in uvec4 in_tex_normal,
	in uvec4 in_tex_metallic,
	in uvec4 in_tex_roughness,
	in uvec4 in_tex_height,
	in vec4 in_blending,
	in vec3 in_upDir,
	in vec3 in_posOnChunk,
	in uint16_t in_flags,
	in uint8_t in_biomes,
	in uint8_t in_features,
	out vec4 out_color,
	out vec3 out_normal,
	out vec2 out_pbr
) {
	...
	vec4 texBlending = GetTextureBlending(in_blending, in_uv, in_tex_height, in_upDir, in_posOnChunk, in_flags, in_biomes, in_features, in_normal, /*out*/triplanarCoords1, /*out*/triplanarCoords2, /*out*/triplanarBlending);
	if ((in_flags & TERRAIN_VERTEX_FLAG_IS_SKIRT) != 0) {
		in_normal = mix(GetVertexNormal(), in_normal, 0.5);
	}
	...
	if (distanceFromEye < textureDisturbanceMaxDistance) {
		out_normal = vec3(0);
		if (texBlending.x > 0) out_normal += texBlending.x * mix(TriplanarLocalNormalMap(in_tex_normal.x, triplanarCoords1, in_normal, triplanarBlending), TriplanarLocalNormalMap(in_tex_normal.x, triplanarCoords2.xyz, in_normal, triplanarBlending), triplanarCoords2.w);
		if (texBlending.y > 0) out_normal += texBlending.y * mix(TriplanarLocalNormalMap(in_tex_normal.y, triplanarCoords1, in_normal, triplanarBlending), TriplanarLocalNormalMap(in_tex_normal.y, triplanarCoords2.xyz, in_normal, triplanarBlending), triplanarCoords2.w);
		if (texBlending.z > 0) out_normal += texBlending.z * mix(TriplanarLocalNormalMap(in_tex_normal.z, triplanarCoords1, in_normal, triplanarBlending), TriplanarLocalNormalMap(in_tex_normal.z, triplanarCoords2.xyz, in_normal, triplanarBlending), triplanarCoords2.w);
		if (texBlending.w > 0) out_normal += texBlending.w * mix(TriplanarLocalNormalMap(in_tex_normal.w, triplanarCoords1, in_normal, triplanarBlending), TriplanarLocalNormalMap(in_tex_normal.w, triplanarCoords2.xyz, in_normal, triplanarBlending), triplanarCoords2.w);
		out_normal = normalize(out_normal);
		
		// Disturb Normal
		vec4 bumpMap = vec4(out_normal, 0.01);
		NormalFromBumpNoise(bumpMap, in_posOnChunk, BumpyNoise)
		vec3 bump = out_normal*bumpMap.a;
		out_normal = normalize(mix(out_normal, bumpMap.xyz, 2.0));
		out_normal = normalize(mix(out_normal, in_normal, smoothstep(textureDisturbanceFadeDistance, textureDisturbanceMaxDistance, distanceFromEye)));
	} else {
	...
	// Metallic / Roughness
	out_pbr = vec2(0);
	if (texBlending.x > 0) out_pbr += texBlending.x * vec2(mix(TriplanarTextureR(in_tex_metallic.x, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_metallic.x, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w), mix(TriplanarTextureR(in_tex_roughness.x, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_roughness.x, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w));
	if (texBlending.y > 0) out_pbr += texBlending.y * vec2(mix(TriplanarTextureR(in_tex_metallic.y, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_metallic.y, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w), mix(TriplanarTextureR(in_tex_roughness.y, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_roughness.y, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w));
	if (texBlending.z > 0) out_pbr += texBlending.z * vec2(mix(TriplanarTextureR(in_tex_metallic.z, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_metallic.z, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w), mix(TriplanarTextureR(in_tex_roughness.z, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_roughness.z, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w));
	if (texBlending.w > 0) out_pbr += texBlending.w * vec2(mix(TriplanarTextureR(in_tex_metallic.w, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_metallic.w, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w), mix(TriplanarTextureR(in_tex_roughness.w, triplanarCoords1, triplanarBlending).r, TriplanarTextureR(in_tex_roughness.w, triplanarCoords2.xyz, triplanarBlending).r, triplanarCoords2.w));
}

void InterpolateTerrainVertices(
	in vec3 barycentricCoords,
	out uvec4 tex_albedo,
	out uvec4 tex_normal,
	out uvec4 tex_metallic,
	out uvec4 tex_roughness,
	out uvec4 tex_height,
	out vec4 blending,
	out vec3 posOnChunk,
	out vec3 upDir,
	out vec3 normal,
	...
	uint i0 = GetVertexIndex(0);
	uint i1 = GetVertexIndex(1);
	uint i2 = GetVertexIndex(2);
	
	TerrainExtraVertexData extras0 = GetTerrainExtraVertexData(i0);
	TerrainExtraVertexData extras1 = GetTerrainExtraVertexData(i1);
	TerrainExtraVertexData extras2 = GetTerrainExtraVertexData(i2);
	
	tex_albedo = extras0.textures + tex_albedo_offset;
	tex_normal = extras0.textures + tex_normal_offset;
	tex_metallic = extras0.textures + tex_metallic_offset;
	tex_roughness = extras0.textures + tex_roughness_offset;
	tex_height = extras0.textures + tex_height_offset;
	
	blending = (
		+ vec4(extras0.blending)/255.0 * barycentricCoords.x
		+ vec4(extras1.blending)/255.0 * barycentricCoords.y
		+ vec4(extras2.blending)/255.0 * barycentricCoords.z
	);
	...
	upDir = normalize(
		+ extras0.upDir * barycentricCoords.x
		+ extras1.upDir * barycentricCoords.y
		+ extras2.upDir * barycentricCoords.z
	);
	...
	tangent = (
		...
		+ GetVertexTangent(i1) * barycentricCoords.y
		+ GetVertexTangent(i2) * barycentricCoords.z
	);
	...
	void main() {
		RCHIT_VISIBILITY_BEGIN
			...
			vec3 barycentricCoords = vec3(1.0f - hitAttribs.x - hitAttribs.y, hitAttribs.x, hitAttribs.y);
				...
				TerrainExtraVertexData extras = GetTerrainExtraVertexData();
				vec3 out_normal;
				ApplyTerrainTextures(
					ray.totalDistanceFromEye,
					normal,
					uv,
					tangent,
					tex_albedo,
			...
			GeometryInstanceData geometry = GetGeometryInstance();
			
			ray.normal = normalize(mat3(MODELVIEW) * out_normal);
...
#if defined(SHADER_VERT) && !defined(SHADER_WIREFRAME)
	#ifndef /*NOT*/USE_TESSELLATION
		DECLARE_TERRAIN_VERTEX_OUTPUTS
	#endif
	void main() {
		if (!IsValidGeometry()) return;
			...
			gl_Position = vec4(GetVertexPosition(), 1);
		#else
			vec4 vertexPosition = vec4(GetVertexPosition(), 1);
			VERT_PREP_MOTION_VECTORS(vertexPosition)
			gl_Position = MVP_AA * vertexPosition;
			TerrainExtraVertexData extras = GetTerrainExtraVertexData();
			out_normal = GetVertexNormal();
			out_uv = GetVertexTexCoord0();
			out_tangent = GetVertexTangent();
			out_tex_albedo = extras.textures + tex_albedo_offset;
			out_tex_normal = extras.textures + tex_normal_offset;
			out_tex_metallic = extras.textures + tex_metallic_offset;
			out_tex_roughness = extras.textures + tex_roughness_offset;
			out_tex_height = extras.textures + tex_height_offset;
			out_blending = vec4(extras.blending) / 255;
			out_posOnChunk = vertexPosition.xyz;
			out_upDir = extras.upDir;
		#endif
	}
#endif
	...
	// // From Sascha Willems
	// const float tessellatedEdgeSize = 100.0;
	...
	// 	vec4 clip0 = (camera.projectionMatrixWithTXAA * (v0 - vec4(radius, vec3(0.0))));
	// 	vec4 clip1 = (camera.projectionMatrixWithTXAA * (v0 + vec4(radius, vec3(0.0))));
	...
	// 	const vec2 viewportSize = vec2(camera.width, camera.height);
	// 	clip0.xy *= viewportSize;
	// 	clip1.xy *= viewportSize;
		
	// 	// Return the tessellation factor based on the screen size 
	// 	// given by the distance of the two edge control points in screen space
	// 	// and a reference (min.) tessellation size for the edge set by the application
	// 	return clamp(distance(clip0, clip1) / tessellatedEdgeSize * camera.tessellation, 1.0, 64.0);
	// }
	
	float GetDistanceTessFactor (vec4 vertex, float minDist, float maxDist, float tess) {
		float dist = length((MODELVIEW * vertex).xyz);
		return clamp(1.0 - (dist - minDist) / (maxDist - minDist), 0.01, 1.0) * tess;
	}
	vec4 DistanceBasedTessellation (vec4 v0, vec4 v1, vec4 v2, float minDist, float maxDist, float tess) {
		vec3 f = vec3(
			GetDistanceTessFactor(v0, minDist, maxDist, tess),
			GetDistanceTessFactor(v1, minDist, maxDist, tess),
			GetDistanceTessFactor(v2, minDist, maxDist, tess)
		);
		return vec4(
			0.5 * (f.y + f.z),
			0.5 * (f.x + f.z),
	...
	bool FrustumCheck() {
		...
		for (int i = 0; i < 3; ++i) {
			vec3 viewSpacePos = (MODELVIEW * vec4(gl_in[i].gl_Position.xyz, 1)).xyz;
			if (dot(viewSpacePos,viewSpacePos) < radius*radius) return true;
			vec4 pos = camera.projectionMatrixWithTXAA * vec4(viewSpacePos, 1);
			pos /= pos.w;
			if (pos.z < 1.0 && abs(pos.x) < 1.0 && abs(pos.y) < 1.0) {
				return true;
			}
		}
		return false;
	...
	void main() {
		if (gl_InvocationID == 0) {
			vec4 tessFactor = vec4(1);
			TerrainExtraVertexData extras = GetTerrainExtraVertexData();
			if ((extras.flags & TERRAIN_VERTEX_FLAG_TESSELLATE) != 0 && camera.tessellation > 0 && FrustumCheck()) {
				tessFactor = DistanceBasedTessellation(gl_in[0].gl_Position, gl_in[1].gl_Position, gl_in[2].gl_Position, tessellationMinDistance, tessellationMaxDistance, camera.tessellation);
			}
	...
	void main() {
		...
		#ifdef SHADER_WIREFRAME
			uvec4 out_tex_albedo;
			uvec4 out_tex_normal;
			uvec4 out_tex_metallic;
			uvec4 out_tex_roughness;
			uvec4 out_tex_height;
			vec4 out_blending;
			vec3 out_posOnChunk;
			vec3 out_upDir;
			vec3 out_normal;
		...
		uint closestIndex = 0;
		if (gl_TessCoord.y > gl_TessCoord.x && gl_TessCoord.y > gl_TessCoord.z) closestIndex = 1;
		else if (gl_TessCoord.z > gl_TessCoord.x && gl_TessCoord.z > gl_TessCoord.y) closestIndex = 2;
		TerrainExtraVertexData extras = GetTerrainExtraVertexData(GetVertexIndex(closestIndex));
	
		// Blending
		vec3 triplanarCoords1;
		vec4 triplanarCoords2;
		vec3 triplanarBlending;
		vec4 texBlending = GetTextureBlending(/*in*/out_blending, /*in*/out_uv, /*in*/out_tex_height, /*in*/out_upDir, /*in*/out_posOnChunk, /*in*/extras.flags, /*in*/extras.biomes, /*in*/extras.features, /*in*/out_normal, /*out*/triplanarCoords1, /*out*/triplanarCoords2, /*out*/triplanarBlending);
		
		// Displacement
		float displacement = 0;
		if (camera.tessellation > 0 && (extras.flags & TERRAIN_VERTEX_FLAG_DISPLACE) != 0) {
			if (texBlending.x > 0) displacement += texBlending.x * mix(TriplanarTextureR(out_tex_height.x, triplanarCoords1, triplanarBlending), TriplanarTextureR(out_tex_height.x, triplanarCoords2.xyz, triplanarBlending), triplanarCoords2.w) * tex0_displacement_factor;
			if (texBlending.y > 0) displacement += texBlending.y * mix(TriplanarTextureR(out_tex_height.y, triplanarCoords1, triplanarBlending), TriplanarTextureR(out_tex_height.y, triplanarCoords2.xyz, triplanarBlending), triplanarCoords2.w) * tex1_displacement_factor;
			if (texBlending.z > 0) displacement += texBlending.z * mix(TriplanarTextureR(out_tex_height.z, triplanarCoords1, triplanarBlending), TriplanarTextureR(out_tex_height.z, triplanarCoords2.xyz, triplanarBlending), triplanarCoords2.w) * tex2_displacement_factor;
			if (texBlending.w > 0) displacement += texBlending.w * mix(TriplanarTextureR(out_tex_height.w, triplanarCoords1, triplanarBlending), TriplanarTextureR(out_tex_height.w, triplanarCoords2.xyz, triplanarBlending), triplanarCoords2.w) * tex3_displacement_factor;
			vertexPosition.xyz += out_normal * clamp(displacement, 0, 1) * tessellationDisplacementFactor;
		}
	
		#ifndef /*NOT*/SHADER_WIREFRAME
			out_pos = MVP * vertexPosition;
			out_pos_history = MVP_HISTORY * vertexPosition;
		#endif
...
#if defined(SHADER_FRAG) && !defined(SHADER_WIREFRAME)
	DECLARE_TERRAIN_FRAGMENT_INPUTS
	DECLARE_DEFAULT_VISIBILITY_FRAGMENT_OUTPUT

	vec3 GetViewSpacePosition() {
		vec4 viewSpacePos = inverse(camera.projectionMatrix) * in_pos;
		viewSpacePos.xyz /= viewSpacePos.w;
		return viewSpacePos.xyz;
	}
	...
	void main() {
		...
		ApplyTerrainTextures(
			GetTrueDistanceFromDepthBuffer(depth),
			normalize(in_normal),
			in_uv,
			in_tangent,
			in_tex_albedo,
		...
		normal = normalize(MODELVIEWNORMAL * normal);
		out_normalAndFresnel = vec4(normal, Fresnel(GetViewSpacePosition(), normal, /*IOR*/1.45));
		out_emission = vec4(GetEmissionColor(), 0/*available*/);
		out_depth = depth;
		FRAG_WRITE_MOTION_VECTORS
	}
#endif
