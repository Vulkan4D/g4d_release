...
void main() {
	...
	const vec3 normal = subpassLoad(in_img_normal).xyz;
	if (dot(normal, normal) == 0) discard;
	
	const vec3 viewSpacePos = GetFragViewSpacePosition();
	if (-viewSpacePos.z > maxDistance) discard;
	...
	for (int i = 0; i < nbSamples; ++i) {
		...
		vec3 viewSpaceSample = viewSpacePos + normalize(offset * dot(offset, normal)) * RandomFloat(seed) * kernelSize;
		vec4 clipSpaceCoord = camera.projectionMatrixWithTXAA * vec4(viewSpaceSample, 1);
		vec2 uv = (clipSpaceCoord.xyz / clipSpaceCoord.w).xy * 0.5 + 0.5;
		float d = GetDepth(uv);
		float sampleDist = -viewSpaceSample.z;
		float sampleDepthDist = GetTrueDistanceFromDepthBuffer(d);
		if (sampleDist > sampleDepthDist) {
			occluded += smoothstep(kernelSize*4, kernelSize, sampleDist - sampleDepthDist)*1.5;
		}
	}
	float ssao = clamp(occluded / float(nbSamples), 0, 1) * clamp(smoothstep(maxDistance, maxDistance/5, -viewSpacePos.z), 0, 1);
	
	vec4 lit = imageLoad(img_lit, ivec2(gl_FragCoord.st));
	imageStore(img_lit, ivec2(gl_FragCoord.st), vec4(mix(lit.rgb, vec3(0), ssao), lit.a));
}
